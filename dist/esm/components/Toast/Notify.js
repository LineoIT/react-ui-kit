...
import { getAccentTheming, getTheming } from './helper';
const intialValue = (toast) => { };
const NotifyContext = React.createContext({
    toastRef: { current: intialValue }
});
export function NotifyProvider({ children }) {
    const toastRef = React.useRef(intialValue);
    return (React.createElement(NotifyContext.Provider, { value: { toastRef } },
        children,
        React.createElement(NotifyContainer, null)));
}
export function NotifyContainer() {
    const [toasts, setToasts] = React.useState([]);
    const { toastRef } = React.useContext(NotifyContext);
    toastRef.current = (_a) => {
    ...
    return (React.createElement("div", { className: "toasts fixed z-99 top-8 right-8" }, toasts.map((t) => (React.createElement(Notify, Object.assign({}, t, { onRemove: onRemove, key: t.id }))))));
}
export function useNotify() {
    const { toastRef } = React.useContext(NotifyContext);
    return {
        push: React.useCallback((toast) => {
