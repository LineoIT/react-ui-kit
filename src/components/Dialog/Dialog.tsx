...
export const ConfirmDialog = React.forwardRef<ConfirmDialogaRef, Props>((prop: Props, ref) => {
    ...
    return (
        <Modal visible={isOpen} setVisible={open} dismissible={false} name={prop.name} overlayClassName="dark:bg-slate-500/80">
            <div className={`w-96 ` + prop.className}>
                <div className="bg-white dark:bg-gray-900 rounded-lg shadow-2xl overflow-hidden">
                    <div className="p-6 space-y-2">
                        <h2 className="text-lg font-bold text-gray-800 dark:text-gray-200">{options?.title ? options?.title : prop.title}</h2>
                        <div className="text-sm text-gray-500 dark:text-gray-400">{options?.description ? options?.description : prop.children}</div>
                    </div>
...
export const ConfirmDialogProvider = ({ children }: { children?: React.ReactNode }) => {
    ...
    return (
        <DialogContext.Provider value={confirmDialog.current}>
            {children}
