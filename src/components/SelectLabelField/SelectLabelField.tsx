...
export function SelectLabelField(
        ...
        ${open ? ` ring-1 border-primary ring-primary` : 'border-gray-400 dark:border-gray-500'}`}
            >
                <div className="flex items-center flex-1">
                    ...
                    <div className="ml-2 text-center flex-1 ">{textCaption && textCaption.length > 25 ? textCaption?.substring(0, 25) + '...' : textCaption}</div>
                </div>
            ...
            <div
                className={`absolute z-[99] text-gray-500 w-full bg-white dark:bg-gray-700 dark:text-gray-300 shadow drop-shadow-2xl  rounded  mt-1 overflow-y-auto transition-all duration-100  ${
                    open ? 'max-h-60  ring-accent ring-1' : 'max-h-0'
                } `}
            >
                {items.map((item, i) => (
                    <div className="flex flex-col divide-y divide-gray-300 w-full" key={i}>
                        <div
                            className={`p-2 cursor-pointer hover:bg-slate-100 dark:hover:bg-accent/10 ${value === item && 'text-accent'}`}
                            onClick={() => {
