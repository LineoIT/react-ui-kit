...
const Item = memo((prop: React.HTMLAttributes<HTMLDivElement>) => {
    ...
    return <div className="p-2 px-4 hover:bg-black/5 cursor-default" {...rest}>{children}</div>
})
...
export const DropButton = ({
    ...
    return <div className="block relative">
        ...
        {active && <div className={`block shadow absolute z-50  overflow-y-auto bg-white dark:bg-slate-900 border-[1px] border-black/[3%] 
    select-none  ${dropClassName}`}>
            ...
            {filtered.map((item, i) => <Item key={i} onClick={() => handleClick(i)}>{item}</Item>)}
        </div>}
    </div>
}
