...
function Toggle(props: ButtonProps) {
  ...
  return <div {...props} onClick={() =>  setOpen(!open)}>{props.children}</div>
}


function Wrapper(props: React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement> & {
  activeClass?: string
  ...
  return  <div {...rest}  className={`absolute overflow-y-auto transition-all duration-200 ${className} ${
    open ? activeClass : desactiveClass
  } `} >
    {props.children}
</div>
