...
export const AccordionHeader: React.FC<React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement>> = (props) => {
    ...
    function toggle(el: HTMLElement) {
        const accordionContent = el.querySelector('.accordion-content') as HTMLDivElement;

        if (accordionContent.style.maxHeight === '0px' || accordionContent.style.maxHeight.length === 0) spend(el);
        else collapse(el);
    }
    ...
    return (
        <div {...props} onClick={handleClick} className={_className}>
            {props.children}
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="fas w-3 h-3">
...
export const AccordionBody: React.FC<React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement>> = (props) => {
    const { className } = props;
...
export const AccordionItem: React.FC<React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement>> = (props) => {
    const { className } = props;
...
export const Accordion: React.FC<React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement>> & {
