...
> = (props) => {
  ...
  let _className = `px-4 ${
    size === "small" ? "py-1" : size === "large" ? "py-3" : size === "none" ? "" : "py-2"
  }
...
export const IconButton: React.FC<
  ...
  >
> = (props) => {
  const { className = "w-10 h-10 py-4 mx-2", children } = props;
  let _className = `ring-0 outline-none flex items-center justify-center rounded-full text-white bg-white-10  cursor-pointer hover:bg-white-20 transition-all duration-300 ease-in-out`;
