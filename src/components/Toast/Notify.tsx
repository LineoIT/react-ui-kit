...
import { getAccentTheming, getTheming, Variant } from './helper';
...
const intialValue = (toast: NotifyOptions) => {};

const NotifyContext = React.createContext({
    toastRef: { current: intialValue }
});

export function NotifyProvider({ children }: React.PropsWithChildren) {
    const toastRef = React.useRef(intialValue);

    return (
        <NotifyContext.Provider value={{ toastRef }}>
            {children}
            <NotifyContainer />
...
export function NotifyContainer() {
    const [toasts, setToasts] = React.useState<NotifyOptions[]>([]);
    const { toastRef } = React.useContext(NotifyContext);

    toastRef.current = ({ timeout = 3000, ...props }) => {
        const id = Date.now();
        const timer = setTimeout(() => {
            setToasts((v) => v.filter((t) => t.id !== id));
        }, timeout);
        setToasts((v) => [...v, { id, timer, ...props } as NotifyOptions]);
    };

    const onRemove = (id: number, timer: number) => {
        clearTimeout(timer);
        setToasts((v) => v.filter((t) => t.id !== id));
    };

    return (
        <div className="toasts fixed z-99 top-8 right-8">
            {toasts.map((t) => (
                <Notify {...t} onRemove={onRemove} key={t.id} />
            ))}
        </div>
    );
}
...
export function Notify(
    ...
    const handleRemove = () => {
        if (onRemove && timer && id) onRemove(id, timer);
    };

    return (
        <div className={`max-w-lg rounded-lg my-5 px-4 py-3 border  shadow-lg animate-toast flex items-center justify-between gap-2 ${getTheming(variant)}`}>
            {ToastIcon(variant)}
            <p className="font-medium flex-1">{props.message}</p>
            <div className="ml-3">
                <button className={`border  rounded-lg p-1 ${getAccentTheming(variant)}`} onClick={handleRemove}>
...
function ToastIcon(variant: Variant) {
    if (variant === 'info') {
        return (
            <svg className="shrink-0 w-6 h-6" viewBox="0 0 16 16">
                <g fill="currentColor">
