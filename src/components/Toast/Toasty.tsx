...
export function useToasty(options: ToastOptions = {
    timeout: 3000,
    closable: true
}) {

    function notify(text: string, variant: Variant = "default", opts: ToastOptions = options) {
        const id = new Date().getTime();
        const theming = getTheming(variant);
        const el = document.createElement("div");
        el.id= id.toString()
        el.className = `max-w-lg rounded-lg  px-4 py-3 border  shadow-lg animate-toast ${theming}`;
    ...
    function ToastProvider() {
        return <div id={toastProviderId} className={`fixed z-40 top-8 right-8`} />;
    }
  
    return {ToastProvider, toast: {notify}};
  }


export const Toasty: React.FC<
...
export function addItemHTML(
    content: string,
    id: any,
    variant: Variant,
    closable?: boolean
  ) {
    const accentTheming = getAccentTheming(variant);
  
    return /* html */`
            <div class="flex items-center gap-2">
            ${
              variant === "default"
                ? /* html */`<svg class="shrink-0 w-6 h-6" viewBox="0 0 16 16">
                  <path
                    fill="currentColor"
          ...
          ${
            variant === "success"
              ? /* html */`<svg class="shrink-0 w-6 h-6" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
            ...
            variant === "warning"
              ...
              ${
                closable
                  ? /* html */`<div class="ml-3">
                    <button
                      class="border  rounded-lg p-1 ${accentTheming}"
...
const getTheming = (variant: Variant) => {
  switch (variant) {
    case "error":
...
const getAccentTheming = (variant: Variant) => {
  switch (variant) {
    case "error":
