...
export const BaseToast: React.FC<React.PropsWithChildren<{
    closable?: boolean;
  ...
  }>>  = ({closable=true ,onClose,timeout= 3000, index, initialValue=false, content, variant="default",className, ...rest}) => {
  ...
  return (<div {...rest}  className={`z-40 top-8 right-8 max-w-lg rounded-lg  px-4 py-3 border  shadow-lg animate-toast ${theming} ${visible ? "visible": "hidden"} ${className}`}>
        ...
        <g fill="currentColor">
          ...
          {closable && <div className="ml-3">
            <button className={`border  rounded-lg p-1 ${accentTheming}`} onClick={handleClose}>
